...
export class AdminBootstrapService {
  ...
  public static initializeSuperAdmin(): AdminAccountRecord {
    ...
    // Keep local development convenient, but never create a production account
    // with a published fallback credential.
    const initialPassword = process.env.INITIAL_ADMIN_PASSWORD || (
      process.env.NODE_ENV === 'production'
        ? crypto.randomBytes(32).toString('base64url')
        : 'SuperAdmin#2026!SecureKey'
    );
    ...
    if (!userDb.has('operator@bulkreach.io')) {
      const opPass = this.hashPassword(
        process.env.NODE_ENV === 'production'
          ? crypto.randomBytes(32).toString('base64url')
          : 'OperatorPass123!'
      );
    ...
    if (!userDb.has('client-demo@enterprise.com')) {
      const userPass = this.hashPassword(
        process.env.NODE_ENV === 'production'
          ? crypto.randomBytes(32).toString('base64url')
          : 'UserDemo123!'
      );
