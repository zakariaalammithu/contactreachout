...
// Derive a 256-bit key from environment. A deterministic fallback is allowed
// only for local development and tests.
function getMasterKey(): Buffer {
  const configuredSecret = process.env.APP_ENCRYPTION_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!configuredSecret && process.env.NODE_ENV === 'production') {
    throw new Error('APP_ENCRYPTION_KEY must be configured in production.');
  }
  const secret = configuredSecret || 'bulkreach-local-development-key';
