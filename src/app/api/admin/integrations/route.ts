  ...
  return NextResponse.json({
    integrations: {
      supabase: {
        ...
        status: process.env.NEXT_PUBLIC_SUPABASE_URL ? 'CHECKING' : 'NOT_CONFIGURED',
      ...
      resend: {
        ...
        status: integrations.RESEND_API_KEY?.configured ? 'CHECKING' : 'NOT_CONFIGURED',
      ...
      googleSheets: {
        ...
        status: integrations.GOOGLE_CLIENT_ID?.configured ? 'CHECKING' : 'NOT_CONFIGURED',
      ...
      openai: {
        ...
        status: integrations.OPENAI_API_KEY?.configured ? 'CHECKING' : 'NOT_CONFIGURED',
      ...
      anthropic: {
        ...
        status: integrations.ANTHROPIC_API_KEY?.configured ? 'CHECKING' : 'NOT_CONFIGURED',
        maskedKey: integrations.ANTHROPIC_API_KEY?.maskedPreview || 'NOT_CONFIGURED',
      },
      stripe: {
        configured: integrations.STRIPE_SECRET_KEY?.configured || false,
        status: integrations.STRIPE_SECRET_KEY?.configured ? 'CHECKING' : 'NOT_CONFIGURED',
        maskedKey: integrations.STRIPE_SECRET_KEY?.maskedPreview || 'NOT_CONFIGURED',
      },
      redis: {
        name: 'Redis BullMQ Queue Broker',
        configured: Boolean(process.env.REDIS_URL || process.env.REDIS_HOST),
        status: Boolean(process.env.REDIS_URL || process.env.REDIS_HOST) ? 'CHECKING' : 'NOT_CONFIGURED',
